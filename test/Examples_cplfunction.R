...
CCPWLfunc1=new(cplfunction,c(-1,2) ,c(-Inf,2,4),0)
CCPWLfunc2=new(cplfunction,c(-1,2),c(-Inf,1,3),0)
CCPWLfunc12=InfConv(CCPWLfunc1,CCPWLfunc2,2)
CCPWLfunc12
CCPWLfunc1=new(cplfunction2,c(-1,2,Inf) ,c(-Inf,2,4),0)
CCPWLfunc2=new(cplfunction2,c(-1,2,Inf),c(-Inf,1,3),0)
CCPWLfunc12=InfConv2(CCPWLfunc1,CCPWLfunc2,2)
CCPWLfunc12

CCPWLfuncList=new(cplfunctionvec) 
CCPWLfuncList$push_back(new(cplfunction,c(-1,1) ,c(-Inf,0,Inf),0))
CCPWLfuncList$push_back(new(cplfunction,c(-1,1) ,c(-Inf,0,Inf),0))

CCPWLfuncList=new(cplfunctionvec) 
n=10000; Y=rnorm(n); S1=array(-1,n);S2=array(1,n); B0=array(-Inf,n); B1=rnorm(n); B2=array(Inf,n);
for (i in 1:n){
  CCPWLfuncList$push_back(new(cplfunction,c(S1[i],S2[i]) ,c(B0[i],B1[i],B2[i]),0))
}
